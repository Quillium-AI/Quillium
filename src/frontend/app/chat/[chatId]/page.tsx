...
export default function ChatDetailPage() {
  return (
    <AuthProvider>
      <ChatDetailPageWithAuth />
    </AuthProvider>
  );
}

function ChatDetailPageWithAuth() {
  const { isAuthenticated, isLoading, error } = useAuth();
  const router = useRouter();
  const params = useParams();
  
  // Extract the chatId from params
...
function ChatDetailContent({ chatId }: ChatDetailContentProps) {
  const router = useRouter();
  ...
  // If the chatId changes through the ChatInterface (unlikely in this route), 
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-900 to-gray-950 text-white">
      ...
      <div className="container mx-auto px-4 py-6 max-w-5xl relative z-10">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <h1 className="text-xl font-semibold hidden sm:block">Quillium Chat</h1>
          
          <div className="w-6"></div> {/* Spacer to balance the flex layout */}
        </div>

        {/* Chat interface */}
        <ChatInterface initialQuery={null} chatId={currentChatId} onChatIdChange={setCurrentChatId} />
      </div>
    </div>
  );
}
